...
const DEFAULT_IMAGE = '/images/newsletter-cleaver.png'

type Props = {
  promo: NewsletterPromo
}

const toMedia = (value: NewsletterPromo['image']): Media | null =>
  value && typeof value === 'object' ? value : null

export const MonthlyMenuPromoClient: React.FC<Props> = ({ promo }) => {
  ...
  return (
    <section
      className="not-prose text-[#f6f3ef]"
      style={{ backgroundColor: sectionBackground || '#141414', color: inputTextColor || '#f6f3ef' }}
    >
        ...
        <div className="flex flex-col justify-center px-8 py-14 md:order-none md:px-12 md:py-16 lg:px-16 lg:py-20">
          {heading ? (
            <h2
              ...
              style={{ color: headingColor || '#f8f6f3' }}
            >
              {heading}
            </h2>
          ) : null}
          {description ? (
            <p
              className="mt-6 max-w-lg text-base leading-relaxed md:text-[17px] md:leading-8"
              style={{ color: bodyTextColor || '#e3ded9' }}
            >
              {description}
            </p>
          ) : null}
            ...
            <div className="absolute inset-y-0 left-[-14%] w-[128%] max-w-none">
              <Image
                ...
                className={[
                  ...
                  grayscaleImage !== false ? 'grayscale' : '',
                ]
                  .filter(Boolean)
                  .join(' ')}
