    ...
    >
      <div className="mx-auto max-w-[1280px] px-8">
        <div className="mb-16 text-center">
          ...
          {subtitle && (
            <p className="mt-4 text-sm uppercase tracking-[0.3em] text-[var(--color-gold)]">{subtitle}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {(items || []).map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="border border-border bg-card p-10 text-center shadow-sm transition hover:border-[var(--color-gold)]/40"
              >
                <Icon className="mx-auto mb-6 h-10 w-10 text-[var(--color-gold)]" strokeWidth={1.5} />
                {item.title && (
